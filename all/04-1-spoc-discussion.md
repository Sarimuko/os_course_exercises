...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   ...
   - 将程序划分为若干功能相对独立的模块，将不会同时执行的模块共享一块内存
   - 不存在调用关系的模块可以互相覆盖

   - 程序员需要合理的将不同的模块分组，从而实现覆盖并最小化内存需求
1. 什么是交换技术？覆盖与交换有什么不同？
   - 将暂时不能使用的程序放到外存
   - 处理的是整个进程

2. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   将不常用的内存块暂存到外存，只讲当前指令需要的部分页或段载入内存，并将暂时不用的换出内存。
