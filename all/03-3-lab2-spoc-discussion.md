...
2. 不同特权级有什么区别？
不同特权级能够访问的数据段不一样，同样的代码的行为也不一样。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
- CPL是code privilege level
- DPL是data privilege level
- RPL是进程访问的request privilege level

- 当指令执行需要访问某个数据段时，需要max(RPL, CPL) <= DPL
- 当函数调用需要访问某个代码段时，需要RPL <= CPL
...
3. int指令在ring0和ring3的执行行为有什么不同？
   ring3时int指令会有特权级切换，因此压栈时会多压SS:ESP。


4. 如何利用int和iret指令完成不同特权级的切换？
   当ring0时，如果修改int时创建的trapframe从而模拟从ring3中断进入ring0的trapframe，当调用iret时即可返回到ring3，从而实现切换。
...
2. 描述X86-32的MMU地址转换过程；
   从虚拟地址先通过段表机制转换成线性地址，然后通过页表转换成物理地址。
